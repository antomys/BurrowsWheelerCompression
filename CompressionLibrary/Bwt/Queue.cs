...
namespace CompressionLibrary.Bwt
{
    ...
    public class PriorityQueue<T> where T : IComparable<T>
    {
        ...
        public void Add(T item)
        {
            ...
            // tree.
            var n = _mA.Count; _mA.Add(item);
            while (n != 0)
            {
                var p = n / 2;    // This is the 'parent' of this item
                if (_mA[n].CompareTo(_mA[p]) >= 0) break;  // Item >= parent
                var tmp = _mA[n]; _mA[n] = _mA[p]; _mA[p] = tmp; // Swap item and parent
                n = p;            // And continue
            }
        }
        ...
        public T Next()
        {
            ...
            _mA[0] = _mA[nMax]; _mA.RemoveAt(nMax);  // Move the last element to the top
